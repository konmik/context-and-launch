...
describe("Settings panel (e2e)", () => {
  ...
  it("keeps column edit dialog open when description PUT fails after rename", async () => {
    ...
    const todoRow = await page.waitForSelector(
      '[data-testid="column-row"][data-column-name="todo"]', { timeout: 3000 },
    );
    ...
    const nameInput = await page.waitForSelector(
      '[data-testid="column-name-input"]', { state: "visible", timeout: 3000 },
    );
    expect(nameInput).toBeTruthy();

    // Change the name (triggers rename flow) and description
    await nameInput!.fill("backlog");
    const descInput = await page.waitForSelector(
      '[data-testid="column-desc-input"]', { state: "visible", timeout: 3000 },
    );
    ...
    await page.waitForSelector(
      '[data-testid="rename-scope-all"]', { state: "visible", timeout: 3000 },
    );
    ...
    const errorEl = await page.waitForSelector(
      '.text-destructive:has-text("description")', { state: "visible", timeout: 5000 },
    );
