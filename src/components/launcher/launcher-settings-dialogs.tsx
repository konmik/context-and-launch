...
function ErrorBanner(props: { message: string }) {
	return <Show when={props.message}><div class="rounded-md bg-destructive/10 px-3 py-2 text-sm text-destructive">{props.message}</div></Show>;
}
...
const itemTypeLabel: Record<ItemType, string> = { template: "Prompt", skill: "Skill", profile: "Launch", shortcut: "Shortcut" };
				...
				{(f) => (<>
					...
					<div class="space-y-3 px-6 py-4">
						<div>
							...
							<input type="text" value={f().name} onInput={(e) => props.setForm({ ...f(), name: e.currentTarget.value })} class="input input-sm" placeholder={f().itemType === "profile" ? "Launch name" : f().itemType === "skill" ? "Skill name" : f().itemType === "shortcut" ? "Shortcut name" : "Prompt name"} />
						</div>
						<div>
							<label class="mb-1 block text-sm text-muted-foreground">{f().itemType === "shortcut" || f().itemType === "profile" ? "Command" : "Prompt"}</label>
							<textarea value={f().text} onInput={(e) => props.setForm({ ...f(), text: e.currentTarget.value })} class="input min-h-[120px]" style={{ height: "auto" }} placeholder={f().itemType === "profile" ? "e.g. powershell -File run-agent.ps1" : f().itemType === "shortcut" ? "e.g. code {{projectPath}}" : "Prompt text with {{placeholders}}"} />
						...
						<Show when={f().mode === "add"}>
							<div>
								...
								<div class="flex gap-4">
									<label class="flex items-center gap-1.5 text-sm"><input type="radio" name="scope" checked={f().scope === "app"} onChange={() => props.setForm({ ...f(), scope: "app" })} /> User</label>
									<label class="flex items-center gap-1.5 text-sm"><input type="radio" name="scope" checked={f().scope === "project"} onChange={() => props.setForm({ ...f(), scope: "project" })} /> Project</label>
								</div>
							</div>
						</Show>
					</div>
					<DialogFooter>
						...
						<button onClick={props.onSubmit} disabled={!f().name.trim()} title={modEnterHint()} class="btn-primary">{f().mode === "add" ? "Add" : "Save"}</button>
					</DialogFooter>
				</>)}
			</Show>
		</DialogRoot>
					...
					<div class="space-y-3 px-6 py-4">
						...
						<div>
							...
							<Show when={props.validation}>
								<p class="mt-1 text-xs text-destructive" data-testid="column-name-error">{props.validation}</p>
							</Show>
						</div>
						<div>
					...
					<div class="space-y-3 px-6 py-4">
						...
						<div class="space-y-2">
							<label class="flex items-center gap-2 text-sm">
								<input type="radio" name="rename-scope" checked={rf().scope === "all"} onChange={() => props.setRenameForm({ ...rf(), scope: "all" })} data-testid="rename-scope-all" />
								All projects using this board
							</label>
							<label class="flex items-center gap-2 text-sm">
								<input type="radio" name="rename-scope" checked={rf().scope === "current"} onChange={() => props.setRenameForm({ ...rf(), scope: "current" })} data-testid="rename-scope-current" />
								Current project only
							</label>
							<label class="flex items-center gap-2 text-sm">
								<input type="radio" name="rename-scope" checked={rf().scope === "none"} onChange={() => props.setRenameForm({ ...rf(), scope: "none" })} data-testid="rename-scope-none" />
			...
			<Show when={props.boardForm}>
				{(bf) => (<>
					...
					<DialogFooter>
						...
						<button onClick={props.onCreate} disabled={!bf().name.trim()} title={modEnterHint()} class="btn-primary" data-testid="board-form-submit">Add</button>
					</DialogFooter>
				</>)}
			</Show>
		</DialogRoot>
		...
		<DialogRoot open={!!props.projectBoardConfirm} onOpenChange={() => props.setProjectBoardConfirm(null)} class="max-w-sm p-0">
			<Show when={props.projectBoardConfirm}>
				{(pbc) => (<>
					...
					<DialogFooter>
						<button onClick={() => props.setProjectBoardConfirm(null)} class="btn-secondary" data-testid="set-project-board-cancel-btn">Cancel</button>
						<button onClick={props.onConfirm} class="btn-primary" data-testid="set-project-board-confirm-btn">Set board</button>
					</DialogFooter>
				</>)}
			</Show>
		</DialogRoot>
