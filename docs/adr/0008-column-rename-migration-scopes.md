...
- Prompt with scope choices. Chosen: gives the user control without forcing a single policy. The "None" option preserves the escape hatch for advanced use cases. Context files are intentionally not renamed -- they are content artifacts and renaming them could break external references.
